...
Leeloo.prototype.cancelJob = function(params, callback) {
	...
	if (id) {
		if (self._processing && self._processing._id && self._processing._id.equals(id)) {
			self._idle();
		}
		...
		self._store.collection(self._context.mongodb.collection, function(error, collection) {
			collection.findAndModify({"_id": new self._store.bson_serializer.ObjectID(params.id), "s": "p"}, [["id", "asc"]], {"$set": {"s": "f"}}, {"safe": true, "upsert": false}, function(error, upd) {
				if (upd) {
					...
					self._broadcast({"command": "finishedJob", "params": upd || {}});
			...
			switch (self._status) {
				case "processing":
					if (self._heartbeats >= self.MAXIMUM_HEARTBEATS_PROCESSING) {
						console.log("[heartbeat] processing " + ((self._processing && self._processing._id) || "") + " (" + self._heartbeats + ")");			
						self._processing && self._processing._id && self.cancelJob({"reason": "timed out", "id": self._processing._d});
						self._idle();
					}
					else {
						console.log("[heartbeat] processing " + ((self._processing && self._processing._id) || "") + " (" + self._heartbeats + ")");			
...
Leeloo.prototype.processJobs = function() {
	...
	self._store.collection(self._context.mongodb.collection, function(error, collection) {
		...
		collection.findOne({"s": "s", "w": {"$lt": Date.now()}}, {"sort": {"w": 1}}, function(error, job) {
			if (error) {
				self.emit("error", error);
			}
			else if (job) {
