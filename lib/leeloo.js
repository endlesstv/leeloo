...
Leeloo.prototype._heartbeat = function(check) {
	...
	if (check) {
		...
		self._store.collection(self._context.mongodb.collection, function(error, collection) {
			if (error) {
				self.emit("error", error, "heartbeat: open collection");
				return;
			}
						
			collection.findOne({"s": "p"}, function(error, job_in_progress) {
				if (error) {
					self.emit("error", error, "heartbeat: find processing job");
					return;
				}
...
Leeloo.prototype._openDatabase = function(callback) {
	if (this._context.mongodb && this._context.mongodb.using_replica_sets) {
		...
		// prepare each server in the replica set
		...
		var repl_set = new ReplSet(servers, {
				"readPreference": Server.READ_SECONDARY, 
				"rs_name": this._context.mongodb.replica_set_name, 
				"socketOptions": {"keepAlive": 1, "timeout": 10000}
			});
		
		// connect to our replica set
		this._store = new Db(this._context.mongodb.datastore, repl_set, {"slaveOk": true, "reaper": true});
	}
	else {
...
Leeloo.prototype.startServer = function(params, callback) {
	...
	self._openDatabase(function(error) {
		...
		self._server.on("error", function(error, location) {
			if (error) {
